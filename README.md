...
There is a hard separation between the stats and the LLM. Python does all the detection, exponential decay fitting for creative fatigue, piecewise regression for frequency saturation, Pearson + Granger for cross-channel cannibalization, Kruskal-Wallis for daypart CVR differences, rolling marginal CPA for budget efficiency. Each module has its own thresholds. The LLM gets the raw statistical results after everything's already been flagged and writes a narrative around the numbers. You could swap the model and nothing changes about what gets detected.
